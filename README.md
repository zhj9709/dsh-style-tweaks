...
- **过程组显示调用次数（默认关闭）**：一轮的思考与工具调用在对话里合并成一个可折叠的过程组，0.1.6 及之前它的标题是一行计数——工具调用次数与消息条数（起了子代理时还带 subagent 数）；0.1.7-alpha.1 把标题换成了用时（生成中 / 失败 / 停止时是各自的状态文案）。开启本项把 0.1.6 的计数接在用时后面，两半同时显示：计数随轮次进行实时增长（多一次工具调用、多一条消息都会立刻反映），失败 / 停止 / 生成中的标题同样适用；没有可计数内容的轮次保持原样。
- **输入框上方恢复蓝色运行状态（默认开启）**：0.1.6 及之前，会话进行中时，对话流末尾、输入框正上方始终有一行独立的蓝色流光状态「深度求索中 / Deep diving」，15 秒后旁边补出实时用时。0.1.7 的工作过程分组重构删掉了这行 React 组件，把运行文案并到过程组标题；进行中的过程组保持展开，标题位于整组最上方，工具调用越多就越容易离开输入框。开启本项按当前语言与宿主实时用时恢复一条视觉状态行；宿主自己的过程组标题和无障碍播报保持不变，不会重复播报。0.1.7 之前的宿主本就有原生状态行，本项保持惰性。
...
style-tweaks:
  ...
  turnProcessCounts: false      # 默认 false；true 则把 0.1.6 的调用次数（工具调用次数 · 消息条数）接在折叠过程组的用时后面
  runningStatus: true           # 默认 true；0.1.7 在输入框上方恢复 0.1.6 的蓝色实时状态行
...
- **输入框上方恢复蓝色运行状态**（`src/client/tweaks/running-status.ts`）：0.1.6 的 `ChatView` 在节点列表之后单独渲染 `TurnStatus`；`running` 时它始终停在对话流末尾，也就是输入框正上方，26 px 单行用品牌蓝渐变扫光，15 秒后再在右侧补一枚普通 caption 色的实时用时。0.1.7 的工作过程分组重构删掉了这个 React 组件：`TurnProcessNodeView` 仍在过程组按钮前放一份视觉隐藏的 `role="status"` 播报（「深度求索中 / Deep diving」），可见标题也仍带实时用时，但进行中的过程组强制展开，标题固定在整组思考与工具行的最上方——调用越多，它离输入框越远，于是旧位置上的蓝色状态像是消失了。本项不搬动宿主节点，也不改过程组：只在当前 `button[data-turn-process]` 的同级无障碍播报是**非结束状态**时认定该轮仍开着，然后把播报原文（而不是强行改写成固定的「深度求索中」）克隆到它的 `[data-chat-flow]` 末尾，挂一个 `div.cst-running-status`。这样如果其它插件把 `Deep diving` 换成 `Deep sleep` / `Deep Sleeping`，本项也会跟着显示新的文案。可见行是宿主当前标题的视觉克隆，并用 `aria-hidden` 排除在无障碍树外；宿主原播报继续负责状态变化，秒级时钟不会重复朗读。用时不做中英字符串解析：把私有标记代入宿主自己生成标题时用的 `message.turnProcess.deepDivingFor` 模板，再从真实标签的相同前后缀之间取值，因此任何语言、任何时长的词序都成立；若文案被其它插件改写，则保留完整自定义标签。挂载行同时用 flex `order` 固定在流末尾，React 后续插入待提交回显时也会被同步回尾部。皮肤逐项移植 0.1.6 的蓝色 shimmer、字号 / 行高 / 时钟排版与 `prefers-reduced-motion` 静态分支。0.1.6 及更早宿主已经在末尾渲染自己的 `TurnStatus`，其过程按钮旁没有这份 0.1.7 同级播报，选择器不会命中；未来若宿主改变两个结构锚点，本项同样保持惰性。
